...
class EngEncoder(nn.Module):
    def __init__(self, eng_vocab_size: int, hidden_state_size: int, longest_sentence, dropout_probability: float = 0.2):
        ...
        self.encoderrnn = nn.LSTM(hidden_state_size,hidden_state_size,batch_first=True)
        self.encoderrnn2 = nn.LSTM(hidden_state_size,hidden_state_size,batch_first=True)
    ...
    def forward(self, input):
        ...
        encoder_output1, (hidden_state1, cell_state1) = self.encoderrnn(embedded)
        encoder_output2, (hidden_state2, cell_state2) = self.encoderrnn2(encoder_output1)

        encoder_output2 = self.batch_norm(encoder_output2)
        encoder_output2 = self.dropout(encoder_output2)

        return encoder_output2, hidden_state1
...
class NlDecoder(nn.Module):
    ...
    def forward_step(self, input, hidden, encoder_outputs):
        ...
        #output2, hidden_state2 = self.decoderrnn2(output1, hidden_state1)
